...
dataset_path = {
    # 'PD': r'E:\2025 组会\3 工作资料\0 张春奎\Article\2 Figure\1 Rats\模型画图\Data\PD_power',
    'LID': r'E:\2025 组会\3 工作资料\0 张春奎\Article\2 Figure\1 Rats\模型画图\Data\LID_power',
    'Nor': r'E:\2025 组会\3 工作资料\0 张春奎\Article\2 Figure\1 Rats\模型画图\Data\SD'
}
...
model.load_state_dict(torch.load(r"E:\2025 组会\3 工作资料\0 张春奎\Article\2 Figure\1 Rats\模型画图\2K_Result\0\M1_DS_Gpe_Tha_Gpi_STN_SNR_PPN.pkl", map_location=torch.device('cpu')))
model = model.to(device)

matrix = np.zeros([test_data.shape[1], 10])

for i in range(test_data.shape[1]):
    test_data_copy = copy.deepcopy(test_data)
    index = np.array(list(range(test_data_copy.shape[0])))

    for j in range(10):
        np.random.shuffle(index)
        test_data_copy[:, i] = test_data_copy[index, i]
        test_dataset = PsdDataset(test_data_copy, test_label)
        ...
        matrix[i, j] = accuracy_wo_open

df = pd.DataFrame(matrix)
df.to_excel('FI_ln.xlsx')
